...
SEED_TOOLS = [
    ...
    # Ollama runs locally — no network = no rate-limits, no auth failures, no
    # 5xx; only model-call errors. Latency is hardware-dependent (mid-range
    # GPU baseline below). Picker tiers cap at "high" so very_high tasks fall
    # through to a paid flagship model.
    ("http://localhost:11434/api/chat", "Ollama (Local)", "llm", 0.97, 1200),
    # Search & Web
...
ALTERNATIVE_GROUPS = {
    ...
    "llm": ["https://api.openai.com/v1/chat/completions", "https://api.anthropic.com/v1/messages", "https://api.groq.com/openai/v1/chat/completions", "https://api.mistral.ai/v1/chat/completions", "https://api.deepseek.com/v1/chat/completions", "http://localhost:11434/api/chat"],
